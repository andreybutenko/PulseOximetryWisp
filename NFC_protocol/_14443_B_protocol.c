...
uint8_t _14443_B_protocol(unsigned char num_bytes) {
    ...
    //toggle_led_2();
    //Begin
    tmp_buffer[count] = rx_buffer[0];
    tmp_buffer[count + 1] = rx_buffer[1];
    tmp_buffer[count + 2] = rx_buffer[2];
    count += 3;

    if (rx_buffer[0] == 0x02) {
    	tmp_buffer[count - 3] = rx_buffer[0];
    ...
    //if ((rx_buffer[0]== 0x05)) { // REQB
        ...
        transmitCommand[index++] = 0xA1;// A: Max FWT(frame waiting time); 4: CID_enable // CID (BIT1) supported and NAD (BIT0) NOT Supported
	...
	////////////////////////////////////////////
        ...
        //Should response an I block with chainning bit not set
            ...
            //Block number == BIT0
        	// Should return 0x3 if receive 0x2
            //transmitCommand[index++] = rx_buffer[0] | block_number;
