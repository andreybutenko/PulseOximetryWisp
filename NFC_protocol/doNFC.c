...
uint8_t debug_interrupt;	// DEBUGGING INTERRUPT
...
#endif
	debug_interrupt = 0;	// DEBUGGING INTERRUPT
}
...
void doRFID_14443B(void) {
	...
	if(doNFC_state&NFC_Start){
		...
		//Whether the start of frame is correct
		if (doNFC_Rx()) {
			if (((rx_buffer[0] == 0x02) || (rx_buffer[0] == 0x03)) && (_14443_buf_ptr >= 2)) {
