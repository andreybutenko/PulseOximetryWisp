...
__interrupt void timeTrackingISR(void) {
	...
	if(doNFC_state<NFC_Tx){
			debug_interrupt = TA0R;	// DEBUGGING INTERRUPT
			TA0CTL = 0;
			TA0R = 0;
			RX_IFG &= ~RX_BIT;
			//debug_interrupt = 1;	// DEBUGGING INTERRUPT
			__bic_SR_register_on_exit(LPM4_bits | GIE);
	//it is in sleep_untill_edge timeout routine
	}else if(doNFC_state&NFC_Start){
			doNFC_state = NFC_Sleep;
			debug_interrupt = 2;	// DEBUGGING INTERRUPT
			__bic_SR_register_on_exit(LPM4_bits | GIE);
	}
}
...
__interrupt void LPM3_TimerISR(void) {
	TA2CTL = 0x00;
	TA2CCTL0 = 0x00;
	debug_interrupt = 4;	// DEBUGGING INTERRUPT
